...
function seedDatabaseIfEmpty() {
  ...
  if (countRow.count > 0) {
    console.log(`[Database] Found ${countRow.count} existing candidates in SQLite.`);
    return;
  }

  console.log('[Database] Seeding database from JSON files...');
  ...
  try {
    ...
    const transaction = db.transaction((candidatesList: Candidate[]) => {
      for (const c of candidatesList) {
        ...
        insertSignals.run(
          c.member.id,
          c.signals.commitDays,
          c.signals.missionsCompleted,
          c.signals.missionsFirstTry,
          c.signals.avgDailyHours || 2.0
        );
      }
    });

    transaction(data.candidates);
    console.log(`[Database] Successfully seeded ${data.candidates.length} candidates into SQLite!`);
  } catch (err) {
...
export function getAllCandidatesDB(): Candidate[] {
  const candidateRows = db.prepare('SELECT * FROM candidates ORDER BY created_at DESC').all() as any[];
  
  const selectMissions = db.prepare('SELECT day, title, passed, skipped, attempts FROM missions WHERE candidate_id = ?');
  const selectSignals = db.prepare('SELECT commit_days, missions_completed, missions_first_try, avg_daily_hours FROM learning_signals WHERE candidate_id = ?');
...
export function saveCandidateDB(candidate: Candidate): void {
  ...
  const transaction = db.transaction(() => {
    ...
    insertSignals.run(
      ...
      candidate.signals.avgDailyHours || 2.0
    );
  });
...
export function saveRecruiterVerdictDB(sessionId: string, verdict: RecruiterVerdict): void {
  ...
  insertOrReplace.run(
    ...
    verdict.verdict,
    verdict.confidence,
    verdict.recruiterNotes,
    JSON.stringify(verdict.evidenceFor),
    JSON.stringify(verdict.evidenceAgainst)
  );
}
